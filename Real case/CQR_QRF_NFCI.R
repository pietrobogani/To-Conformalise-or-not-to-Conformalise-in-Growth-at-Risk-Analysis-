# This script replicates the analysis of Adrian et al., but implementing the CQR QRF algorithm instead of QR
...
set.seed(3) # QRF algorithms needs a seed in order to produce always the same result
...
if (loadsavedresults == FALSE) {
  ...
  for (jt in jtFirstOOS:(length(Time) - h)) { 
    ...
    x0 <- Z[1:test_length,-1]
    x1 <- Z[(test_length+1):(jt - h),-1] 
    x_test <- t(as.matrix(Z[jt,-1]))
    
    xGDPonly0 <- as.matrix(ZGDPonly[1:test_length,-1])
    xGDPonly1 <- as.matrix(ZGDPonly[(test_length+1):(jt - h),-1])
    xGDPonly_test <- as.matrix(as.numeric(ZGDPonly[jt,-1]))
        
    YQ_OOSCO[jt + h, ] <- qCQRF_opposite(x0,y0,x1,y1,x_test,QQ)
    YQGDPonly_OOSCO[jt + h, ] <- qCQRF_opposite(xGDPonly0,y0,xGDPonly1,y1,xGDPonly_test,QQ)
...
if(loadsavedresults == FALSE) { # Save results
  ...
  filename <- paste("ResOOS_CQR_QRF_H", h, ".RData", sep="")
...
p <- ggplot(df, aes(x = Quantile, y = EmpiricalCoverage, color = Group)) +
  ...
  scale_color_manual(values = c("CQR QRF NFCI" = cqr_colors, "CQR QRF GDPonly" = qr_colors)) + # Manual color scale
